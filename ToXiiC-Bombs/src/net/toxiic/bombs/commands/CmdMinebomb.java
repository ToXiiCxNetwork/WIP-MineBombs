        ...
        if (args.length == 4)
        {
          if (args[0].equalsIgnoreCase("give"))
          {
            ...
            if (targetPlayer != null) {
              LegitMainUtil.giveMinebomb(amount, power, new Player[] { targetPlayer });
            } else if ((args[1].equals("*")) || (args[1].equals("**"))) {
              for (World w : Bukkit.getWorlds())
              {
                List<Player> worldPlayers = w.getPlayers();
                LegitMainUtil.giveMinebomb(amount, power, (Player[])worldPlayers.toArray(new Player[worldPlayers.size()]));
              }
            }
          }
        }
